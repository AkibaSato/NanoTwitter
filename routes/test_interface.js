...
router.post('/reset/standard', async function (req, res, next) {
  await resetAll();
  var tweetN=req.query['tweets']
  await Loader.loadData(req, res, tweetN)
...
router.post('/users/create', function (req, res, next) {
    ...
    if(count && tweets) {
      ...
      Loader.fakeUserTweet(req, res, 1, 0)
    }
});
...
router.post('/user/:id/tweets', function (req, res, next) {
      ...
      if (id==testuser && tweets) {
        ...
        Loader.createNTweets(req, res, req.params.id, tweets)
      }
});
...
async function resetTestUser() {
 testuser=await User.create(test_param)
 return testuser;
};
